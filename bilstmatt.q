...
Lwx:((2,hsz,(1+esz))#sampler[master;1000];(2,hsz,(1+esz))#sampler[master;1000];(2,hsz,(1+esz))#sampler[master;1000];(2,hsz,(1+esz))#sampler[master;1000]);
...
decoder:{[oSeq]
	...
	while[t<count oSeq;
		...
		alphasT:mlpFw[(sPrev,cPrev);smp];
		ctx:sum over alphasT*annot;
		/ Get word embedding
		h:embFW[DCDR;oSeq[t]];
		dinput:0;
		$[-1=smp:dinput:(ctx,smp);dinput:(h,ctx,smp)]
		h:decoderFw[DCDR;dinput];
		h:softmaxfw[h];
		t:t+1
		 ]
	}
